...
const create = (client) => {
  ...
  const deploy = (stackname, version, envFilePath, tagFilePath) => {
    ...
    const paramsPromise = existingParamsPromise.then(params => Object.assign({}, params, {[VERSION_PARAM]: version}));
  ...
  const update = (stackname, version, templateFilePath, paramsFilePath, envFilePath, tagFilePath) => {
    const templatePromise = utils.read(templateFilePath).then(JSON.parse);

    const paramsPromise = getParamsFromFile(paramsFilePath, {[VERSION_PARAM]: version});
    const envPromise = getEnv(envFilePath);
    const tagPromise = getTags(tagFilePath);
    const existingParamsPromise = cf.getParameters(stackname);
