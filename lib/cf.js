...
const create = (client) => {
  ...
  const waitForUpdate = (name) => {
    return waitFor(name, 'stackUpdateComplete').promise();
  };

  const waitForCreate = (name) => {
    return client.waitFor(name, 'stackCreateComplete').promise();
  };

  const waitForDelete = (name) => {
    return client.waitFor(name, 'stackDeleteComplete').promise();
  };
